...
if isempty(players)
    warning("dfs:emptyPool", ...
        "Every row was filtered out (%d injured/out, %d with no projection). " + ...
        "The export may be incomplete; try ExcludeInjury=[] or DropZeroProjection=false to inspect it.", ...
        sum(ismember(Injury, upper(opts.ExcludeInjury))), sum(~(Projection > 0)));
end
